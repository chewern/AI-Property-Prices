...
def main():
    ...
    # MODELS ASSEMBLY for original price vs unskewed price
    ######################################################
    model_OLS = make_pipeline(preprocess,
        LinearRegression()
    )
    
    model_OLS_unskewed = make_pipeline(preprocess,
        LinearRegression()
    )

    # TRAIN & SCORE MODEL_OLS, printout and save to file
    #####################################################
    model_name = 'OLS_no_unskewing'

    # Train/Test Split
    x_train, x_test, y_train, y_test = train_test_split(features, output_var, 
                                                        random_state=42, test_size=0.25)

    model_OLS.fit(x_train, y_train)
    pred_test = np.round(model_OLS.predict(x_test))

    # calculate the scores and difference
    rmse = np.sqrt(mean_squared_error(y_test, pred_test))
    r2 = r2_score(y_test, pred_test)
    difference = np.round(y_test - pred_test)
    ...
    # TRAIN & SCORE UNSKEWED MODEL, printout and save to file
    #####################################################
    model_name = 'OLS_unskewed'

    # Train/Test Split
    x_train, x_test, y_train_log, y_test_log = train_test_split(features, output_var_log, 
                                                        random_state=42, test_size=0.25)

    model_OLS_unskewed.fit(x_train, y_train_log)
    pred_test_log = model_OLS_unskewed.predict(x_test)
    
    #inverse log to get the price, round to whole numbers
    y_test = np.round(np.exp(y_test_log), decimals=0)
    pred_test = np.round(np.exp(pred_test_log), decimals=0)
