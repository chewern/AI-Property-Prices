...
def main():
    ...
    params = {'ridge__alpha' : [0.01, 0.1, 1, 2, 5]}

    model = GridSearchCV(pipeline, params, cv=8, scoring='neg_mean_squared_error') 
    ...
    print("R2 Score: {:.4f}\n".format(r2))
    compare = pd.DataFrame(list(zip(y_test, pred_test, difference)),columns=['Actual Sale Price','Predicted Sale Price','Difference'])
