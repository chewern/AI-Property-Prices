...
def get_age(df):
    ...
    for i, yr in df.renovation.items(): 
        if yr == 0:
            df.loc[i,'renovation'] = df.loc[i,'built'] 
        else: 
...
class GetAge(BaseEstimator, TransformerMixin):
    ...
    def transform(self,DF):
        transaction_year = DF['date'].dt.year
        DF['built'] = (transaction_year - DF['built']).astype(np.int32)
        #DF = DF.rename(columns={'built':'age'})
        ...
        DF['renovation'].astype(np.int32)
        #renaming renovation to reno_age
        #DF = DF.rename(columns={'renovation':'reno_age'})
        
        return DF
    
class GetMonth(BaseEstimator, TransformerMixin):
    """Custom Transformer: """
...
def results_print(model_name, accuracy, precision, recall, f1):
    ...
    print("###############################################################")
    print(model_name + " using 11 bins/classes")
    print("========================")
...
def save_to_file(txt_filename, model_name, accuracy, precision, recall, f1):
    ...
    with open("data/" + txt_filename, "a") as f:  #'a' for append, 'w' to write
        print("############################################", file=f)
        print(model_name + ' using 11 bins', file=f)
...
def main():
    ...
    numerical_features = ['bedrooms','bathrooms','floors',
                          'view','condition','review_score','basement_size',
                          'built','renovation','living_room_size','lot_size'] # TASK: Define numerical column names
    categorical_features = ['zipcode','date']
    binary_features = ['waterfront']
    
    preprocess = make_column_transformer(
       # (GetAge(),feature_names), 
        (StandardScaler(),numerical_features), 
        (GetMonth(),categorical_features), 
    ...
    model_MLP = make_pipeline(preprocess,
        MLPClassifier(hidden_layer_sizes=30, alpha=0.01, random_state=1)
    )
    ...
    for model_name, clf in clfs:
        ...
        txt_filename = 'Classifications_Assemble.txt'
